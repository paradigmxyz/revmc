...
tests! {
    ...
    // Tests that known-constant operand snapshots are correctly used during translation.
    // These exercise the `operand_value_or_load` path in popn/dup/exchange.
    const_snapshot {
        // Arithmetic with known constants.
        push_pop(@raw {
            bytecode: &[op::PUSH1, 42, op::POP],
            expected_gas: 3 + 2,
        }),
        add(@raw {
            bytecode: &[op::PUSH1, 10, op::PUSH1, 20, op::ADD],
            expected_stack: &[30_U256],
            expected_gas: 3 + 3 + 3,
        }),
        sub(@raw {
            bytecode: &[op::PUSH1, 10, op::PUSH1, 30, op::SUB],
            expected_stack: &[20_U256],
            expected_gas: 3 + 3 + 3,
        }),
        mul(@raw {
            bytecode: &[op::PUSH1, 6, op::PUSH1, 7, op::MUL],
            expected_stack: &[42_U256],
            expected_gas: 3 + 3 + 5,
        }),
        chain_add_mul(@raw {
            bytecode: &[op::PUSH1, 3, op::PUSH1, 4, op::ADD, op::PUSH1, 2, op::MUL],
            expected_stack: &[14_U256],
            expected_gas: 3 + 3 + 3 + 3 + 5,
        }),

        // DUP with known constants.
        dup1(@raw {
            bytecode: &[op::PUSH1, 42, op::DUP1],
            expected_stack: &[42_U256, 42_U256],
            expected_gas: 3 + 3,
        }),
        dup2(@raw {
            bytecode: &[op::PUSH1, 11, op::PUSH1, 22, op::DUP2],
            expected_stack: &[11_U256, 22_U256, 11_U256],
            expected_gas: 3 + 3 + 3,
        }),
        dup1_add(@raw {
            bytecode: &[op::PUSH1, 21, op::DUP1, op::ADD],
            expected_stack: &[42_U256],
            expected_gas: 3 + 3 + 3,
        }),
        push32_dup(@raw {
            bytecode: &{
                let mut code = [0u8; 35];
                code[0] = op::PUSH32;
                code[1..33].copy_from_slice(&U256::MAX.to_be_bytes::<32>());
                code[33] = op::DUP1;
                code[34] = op::STOP;
                code
            },
            expected_stack: &[U256::MAX, U256::MAX],
            expected_gas: 3 + 3,
        }),

        // SWAP/exchange with known constants.
        swap1(@raw {
            bytecode: &[op::PUSH1, 1, op::PUSH1, 2, op::SWAP1],
            expected_stack: &[2_U256, 1_U256],
            expected_gas: 3 + 3 + 3,
        ...
        dup1_swap1(@raw {
            bytecode: &[op::PUSH0, op::DUP1, op::SWAP1],
            expected_stack: &[U256::ZERO, U256::ZERO],
            expected_gas: 2 + 3 + 3,
        }),
        swap_add(@raw {
            bytecode: &[op::PUSH1, 1, op::PUSH1, 2, op::PUSH1, 3, op::SWAP1, op::ADD],
            expected_stack: &[1_U256, 5_U256],
            expected_gas: 3 + 3 + 3 + 3 + 3,
        }),

        // Mixed const/non-const operands.
        address_add(@raw {
            bytecode: &[op::ADDRESS, op::PUSH1, 1, op::ADD],
            expected_stack: &[{
                let addr: U256 = DEF_ADDR.into_word().into();
                addr + 1_U256
            }],
            expected_gas: 2 + 3 + 3,
        }),
        address_swap(@raw {
            bytecode: &[op::PUSH1, 42, op::ADDRESS, op::SWAP1],
            expected_stack: &[DEF_ADDR.into_word().into(), 42_U256],
            expected_gas: 3 + 2 + 3,
        }),
        address_dup1(@raw {
            bytecode: &[op::ADDRESS, op::DUP1],
            expected_stack: &[DEF_ADDR.into_word().into(), DEF_ADDR.into_word().into()],
            expected_gas: 2 + 3,
        }),
        ...
        // Comparison ops.
        iszero(@raw {
            bytecode: &[op::PUSH1, 0, op::ISZERO],
            expected_stack: &[1_U256],
            expected_gas: 3 + 3,
        }),
        iszero_nonzero(@raw {
            bytecode: &[op::PUSH1, 5, op::ISZERO],
            expected_stack: &[0_U256],
            expected_gas: 3 + 3,
        }),
        lt(@raw {
            bytecode: &[op::PUSH1, 10, op::PUSH1, 5, op::LT],
            expected_stack: &[1_U256],
            expected_gas: 3 + 3 + 3,
        }),
        eq(@raw {
            bytecode: &[op::PUSH1, 42, op::PUSH1, 42, op::EQ],
            expected_stack: &[1_U256],
            expected_gas: 3 + 3 + 3,
        }),
        eq_neq(@raw {
            bytecode: &[op::PUSH1, 42, op::PUSH1, 43, op::EQ],
            expected_stack: &[0_U256],
            expected_gas: 3 + 3 + 3,
        }),
        ...
        // Bitwise ops.
        not(@raw {
            bytecode: &[op::PUSH1, 0, op::NOT],
            expected_stack: &[U256::MAX],
            expected_gas: 3 + 3,
        }),
        and(@raw {
            bytecode: &[op::PUSH1, 0xff, op::PUSH1, 0x0f, op::AND],
            expected_stack: &[0x0f_U256],
            expected_gas: 3 + 3 + 3,
        }),
        or(@raw {
            bytecode: &[op::PUSH1, 0xf0, op::PUSH1, 0x0f, op::OR],
            expected_stack: &[0xff_U256],
            expected_gas: 3 + 3 + 3,
        }),
        shl(@raw {
            bytecode: &[op::PUSH1, 1, op::PUSH1, 4, op::SHL],
            expected_stack: &[16_U256],
            expected_gas: 3 + 3 + 3,
        }),
        shr(@raw {
            bytecode: &[op::PUSH1, 16, op::PUSH1, 2, op::SHR],
            expected_stack: &[4_U256],
            expected_gas: 3 + 3 + 3,
        }),

        // Fallible builtins (MSTORE, SLOAD).
        mstore(@raw {
            bytecode: &[op::PUSH1, 0x42, op::PUSH0, op::MSTORE],
            expected_memory: &0x42_U256.to_be_bytes::<32>(),
            expected_gas: 3 + 2 + (3 + memory_gas_cost(1)),
        }),
        sload(@raw {
            bytecode: &[op::PUSH1, 69, op::SLOAD],
        ...
        // Control flow.
        jump(@raw {
            bytecode: &[op::PUSH1, 3, op::JUMP, op::JUMPDEST, op::PUSH1, 99],
            expected_stack: &[99_U256],
            expected_gas: 3 + 8 + 1 + 3,
        }),
        jumpi_false(@raw {
            bytecode: &[op::JUMPDEST, op::PUSH0, op::PUSH0, op::JUMPI, op::PUSH1, 77],
            expected_stack: &[77_U256],
            expected_gas: 1 + 2 + 2 + 10 + 3,
        }),
    }
