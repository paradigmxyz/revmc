    ...
    fn run_fixpoint(
        ...
        loop {
            let Some(bid) = worklist.pop() else {
                ...
                // Find a root: Bottom, no predecessors.
                ...
                let push_block_with_fake_stack =
                    |this: &Self,
                     block_states: &mut IndexVec<_, _>,
                     worklist: &mut Worklist,
                     bid| {
                        let block = &this.cfg.blocks[bid];
                        let n = this.block_min_input_depth(block);
                ...
                if let Some(root) = root {
                    ...
                    push_block_with_fake_stack(self, block_states, &mut worklist, root);
                    last_root = root;
                    continue;
                }
                ...
                for bid in self.cfg.blocks.indices() {
                    if matches!(block_states[bid], BlockState::Bottom) {
                        push_block_with_fake_stack(self, block_states, &mut worklist, bid);
...
mod tests_edge_cases {
    ...
    fn non_converged_multi_jump_stays_dynamic() {
        ...
        //
        // The iteration budget is capped at 100_000. Each call site propagates through
        // all b relay blocks, so iterations ≈ k*b. We need k*b > 100_000.
        let k = 200;
        let b = 600;
