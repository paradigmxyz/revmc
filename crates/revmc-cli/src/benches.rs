...
#[derive(Clone, Debug)]
pub enum BenchKind {
    Bytecode(BytecodeBenchDef),
    TxFixture(FixtureBenchDef),
}

/// Definition for a bytecode benchmark.
#[derive(Clone, Debug, Default)]
pub struct BytecodeBenchDef {
    pub bytecode: Vec<u8>,
    pub calldata: Vec<u8>,
...
impl Bench {
    /// Returns the bytecode bench definition, if this is a bytecode bench.
    pub fn as_bytecode(&self) -> Option<&BytecodeBenchDef> {
        match &self.kind {
            BenchKind::Bytecode(def) => Some(def),
